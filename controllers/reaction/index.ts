...
export async function findReactionsByNoteAndDelete(noteId: string): Promise<any> {
  try {
    const reactionsList = await getReactionsByNote(noteId);
    if(!reactionsList?.length){
      return;
    }
    const deleted = reactionsList.reduce(async (promise: Promise<any>, note: {[Key: string]: any}) => {
      await promise;
